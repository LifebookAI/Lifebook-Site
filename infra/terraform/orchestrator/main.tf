...
resource "aws_cloudwatch_log_group" "lg" { name="/aws/lambda/${local.name_prefix}-worker", retention_in_days=60 }
...
resource "aws_lambda_function" "worker" {
  ...
  environment { variables = { LF_ORCH_QUEUE_URL = aws_sqs_queue.queue.url, LF_ORCH_TABLE = aws_dynamodb_table.jobs.name, NODE_OPTIONS="--enable-source-maps" } }
...
resource "aws_iam_role_policy" "events_to_sqs" {
  ...
  policy = jsonencode({ Version="2012-10-17", Statement=[{ Effect="Allow", Action=["sqs:SendMessage"], Resource=aws_sqs_queue.queue.arn }] })
}

resource "aws_cloudwatch_event_rule" "hourly" { name="${local.name_prefix}-hourly", schedule_expression="rate(1 hour)" }
